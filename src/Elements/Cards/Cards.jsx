...
const Cards = () => {
  ...
  useEffect(() => {
    ...
    fetchTrainingData();
    if (userRole==="viewer"){
      fetchWfoData(); // Fetch WFO data only for viewers
    }
  ...
  // const fetchWfoData = () => {
  //   const loggedInUserName = `${userName}(${UserEmpId})`; // Construct the full name format

  //   console.log(loggedInUserName);
  //   axios
  //     .get("http://localhost:8000/employeeAttendances") // Updated endpoint
  //     .then((response) => {
  //       const userAttendanceData = response.data.find(
  //         (record) => record.name === loggedInUserName
  //       );

  //       if (userAttendanceData) {
  //         setTo(userAttendanceData.TO);
  //         setTh(userAttendanceData.TH);
  //         setTl(userAttendanceData.TL);
  //         console.log(userAttendanceData.TO);
  //         console.log(userAttendanceData.TH);
  //         console.log(userAttendanceData.TL);
  //       }
  //     })
  //     .catch((error) => {
  //       console.error("Error fetching WFO data: ", error);
  //     });
  // };

  const fetchWfoData = () => {
    const loggedInUserName = userName.trim(); // Get only the user name part

    console.log("Logged-in User Name:", loggedInUserName);
    axios
      .get("http://localhost:8000/employeeAttendances") // Updated endpoint
      .then((response) => {
        const userAttendanceData = response.data.find((record) => {
          // Extract the name part from record.name (before '(')
          const recordName = record.name.split("(")[0].trim();
          return recordName === loggedInUserName;
        });
        ...
        if (userAttendanceData) {
          ...
          setTh(userAttendanceData.TH);
          setTl(userAttendanceData.TL);
          console.log("TO:", userAttendanceData.TO);
          console.log("TH:", userAttendanceData.TH);
          console.log("TL:", userAttendanceData.TL);
        }
      })
      .catch((error) => {
        console.error("Error fetching WFO data: ", error);
        ...
        >
          <div className="flex flex-col gap-3 items-center justify-center h-full">
            ...
            <div style={{ fontSize: textStyle.contentFontSize }}>
              {userRole === "viewer" ? `${to} / ${th} / ${tl}` : "0 / 0 / 0"}
            </div>
            <div style={{ fontSize: "25px" }}>WFO/WFH/Leave</div>
          </div>
        </Card>
